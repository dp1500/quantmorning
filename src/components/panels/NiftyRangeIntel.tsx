...
/** Groups Model 1/2 upper+lower into one clean tooltip line */
      ...
      {m1Med != null && (
        <p className="text-amber-400">
          ...
          <span className="text-text-tertiary ml-1">
            ({Math.round(m1Hi)?.toLocaleString()} – {Math.round(m1Lo)?.toLocaleString()})
          </span>
        </p>
      )}
      {m2Med != null && (
        <p className="text-blue-400">
          ...
          <span className="text-text-tertiary ml-1">
            ({Math.round(m2Hi)?.toLocaleString()} – {Math.round(m2Lo)?.toLocaleString()})
          </span>
        </p>
      )}
    </div>
...
const NiftyRangeIntel: React.FC<{ data?: GarchForecast; oiAnalysis?: any }> = ({
  ...
  // ── GARCH chart data (last 3 actual + 5 forecast) ──
  const forecastChartData = useMemo(() => {
    if (!forecastData) return [];
    const rows: any[] = [];
    const last3 = (forecastData.last5Days || []).slice(-3);
    last3.forEach((d: any) => {
      rows.push({
        date: d.date?.slice(5),
        actual: d.actual,
        m1Median: null, m1Lower: null, m1Upper: null,
        m2Median: null, m2Lower: null, m2Upper: null,
        isForecast: false,
      });
    });
    (forecastData.forecast5Days || []).forEach((d: any) => {
      rows.push({
        ...
        m2Median: d.garchMedian, m2Lower: d.garchLower, m2Upper: d.garchUpper,
        isForecast: true,
      });
    });
    return rows;
  ...
  const spotPrice = forecastData?.last5Days?.length
    ? forecastData.last5Days[forecastData.last5Days.length - 1].actual
    : 0;
        ...
        <div className="lg:col-span-2">
          <ResponsiveContainer width="100%" height={FORECAST_HEIGHT}>
            <ComposedChart data={forecastChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <Legend wrapperStyle={{ fontSize: 10, color: CHART_COLORS.textTertiary }} iconType="line" iconSize={10} />

              {/* Actual */}
              <Line type="monotone" dataKey="actual" stroke={CHART_COLORS.primary}
                strokeWidth={2.5} dot={{ r: 4, fill: CHART_COLORS.primary, strokeWidth: 0 }}
              ...
              {/* Model 1 median */}
              <Line type="monotone" dataKey="m1Median" stroke="#f59e0b"
                strokeWidth={2} dot={{ r: 3, fill: '#f59e0b', strokeWidth: 0 }}
                name="Model 1" connectNulls
              />
              {/* Model 2 band */}
              ...
              {/* Model 2 median */}
              <Line type="monotone" dataKey="m2Median" stroke="#3b82f6"
                strokeWidth={2} strokeDasharray="4 3" dot={{ r: 3, fill: '#3b82f6', strokeWidth: 0 }}
                name="Model 2" connectNulls
              />
              {/* Separator at forecast start */}
              {forecastChartData.length > 3 && (
                <ReferenceLine x={forecastChartData[2]?.date} stroke={CHART_COLORS.textTertiary}
                  strokeDasharray="1 2" strokeOpacity={0.3} />
              )}
            </ComposedChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="lg:col-span-1 flex flex-col">
          {expiries.length > 1 && (
            <div className="flex items-center gap-1 mb-2">
              {expiries.map((exp: any, i: number) => (
                ...
                >
                  {exp.expiryDate?.slice(5) || `Expiry ${i + 1}`}
                </button>
              ))}
            </div>
          )}
          ...
          {oiBarData.length > 0 ? (
            <div className="flex-1">
              <ResponsiveContainer width="100%" height={OI_HEIGHT}>
                <BarChart data={oiBarData} layout="vertical" margin={{ top: 5, right: 10, left: 0, bottom: 5 }}
                  barGap={2} barCategoryGap={6}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_COLORS.border} strokeOpacity={0.2} horizontal={false} />
                  <XAxis type="number" tickFormatter={formatOI}
                    tick={{ fontSize: 9, fill: CHART_COLORS.textTertiary }} tickLine={false} axisLine={false}
                  />
                  ...
                  />
                  <Bar dataKey="putOI" fill="#ef4444" fillOpacity={0.55} barSize={6} radius={[0, 2, 2, 0]} name="Put OI" />
                  <Bar dataKey="callOI" fill="#22c55e" fillOpacity={0.55} barSize={6} radius={[2, 0, 0, 2]} name="Call OI" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ) : (
            <div className="flex items-center justify-center flex-1 min-h-[200px] text-xs text-text-tertiary">
              No OI data
            </div>
          )}
          ...
          {activeExpiry && (
            <div className="flex items-center gap-3 text-[10px] text-text-tertiary font-mono border-t border-border-primary pt-2 mt-1">
              <span>
                PCR <span className="text-text-secondary">{activeExpiry.pcr}</span>
              </span>
            </div>
          )}
        </div>
